...
--1. Liệt kê các đơn hàng gửi tới Anh Mĩ Nhật (337)
SELECT *
FROM Orders
WHERE ShipCity in ('UK','USA','Japan')
...
SELECT * 
FROM Employee
WHERE not (City = 'London' OR City = 'NY') 

SELECT * 
FROM Employee
WHERE City not between 'London' and 'NY'
...
SELECT *
FROM Orders
WHERE not (Freight < 50 AND Freight > 100)
...
SELECT *
FROM Orders
WHERE not not not (Freight < 50 AND Freight > 100)
--between là đẹp nhất
...
SELECT *
FROM Orders
WHERE YEAR(ShipAddress) = 2021 AND MONTH(ShipAddress) not between 6 and 9

SELECT *
FROM Orders
WHERE YEAR(ShipAddress) = 2021 AND MONTH(ShipAddress) not in (6 ,7 ,8 ,9)
